    ...
    <header className="bg-midnight-900/95 backdrop-blur-md border-b border-ice-300/10 shadow-md">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            <div className="flex items-baseline">
              <span className="text-xl lg:text-3xl font-bold tracking-tight text-ice-100 playful-text animate-glow mr-2">qUALMS</span>
              <span className="text-sm lg:text-base text-lavender font-mono font-semibold">[kʰwɑːmz]</span>
            </div>
            <span className="text-sm lg:text-base text-ice-100 font-medium tracking-wide">
            ...
            {navItems.map((item) =>
              item.dropdown ? (
                <div key={item.name} className="relative group">
                  <button className="px-3 py-2 text-base lg:text-lg text-ice-300 hover:text-ice-100 hover:bg-lavender/10 rounded artistic-text capitalize">
                    {item.name}
                  </button>
                  <div className="absolute left-0 top-full mt-1 hidden group-hover:block bg-midnight-900/95 backdrop-blur-md rounded shadow-md z-10">
                    {item.dropdown.map((subItem) => (
                      <Link
                        key={subItem.name}
                        href={subItem.link}
                        className="block px-4 py-2 text-sm text-ice-300 hover:bg-lavender/10 hover:text-ice-100 capitalize artistic-text"
                      >
                        {subItem.name}
                      </Link>
                    ))}
                ...
                <Link
                  key={item.name}
                  href={item.link!}
                  className={`px-3 py-2 text-base lg:text-lg transition-colors duration-200 capitalize rounded artistic-text ${
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden bg-midnight-900/95 backdrop-blur-md">
          {/* this will show animated text on mobile too */}
            ...
            {navItems.map((item) =>
              item.dropdown ? (
                <div key={item.name} className="w-full">
                  <button className="w-full text-left px-3 py-2 text-lg text-ice-300 capitalize hover:bg-lavender/10 artistic-text">
                    {item.name}
                  </button>
                  {item.dropdown.map((subItem) => (
                    <Link
                      key={subItem.name}
                      href={subItem.link}
                      className="block pl-8 pr-3 py-2 text-lg text-ice-300 capitalize hover:bg-lavender/10 artistic-text"
                    >
                      {subItem.name}
                    </Link>
                  ))}
                ...
                <Link
                  key={item.name}
                  href={item.link!}
                  className="px-3 py-2 text-lg text-ice-300 capitalize hover:bg-lavender/10 rounded artistic-text"
                >
                  {item.name}
                </Link>
              )
